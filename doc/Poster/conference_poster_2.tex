...
\begin{itemize}
	...
	\begin{itemize}
	\item The human segmentation is based on the assumption that human heads are distinguishable modes in the depth image and that people moving very close to each other seldom differ much more than a head in height (figure 3). The segmentation is realized by a series of threshold and morphological operations, Gaussian blurring, contour drawing and searches for local maxima's (figure 2).
	\end{itemize}
\end{itemize}
\begin{figure}
...
\begin{itemize}
	...
	\begin{itemize}
		\item Queues are detected using Bézier splines based on object velocities (fig 4).
